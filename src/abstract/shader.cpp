...
void shader_set_uniform3f( Shader *shader, const char *name, glm::vec3 &vec ) {
    shader_bind( shader );
    glUniform3fv( get_uniform_location( shader, name ), 1, &vec[ 0 ] );
}
void shader_set_uniformf( Shader *shader, const char *name, float f ) {
    shader_bind( shader );
