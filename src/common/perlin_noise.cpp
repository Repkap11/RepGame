...
float noise3d( float x, float y, float z, int seed ) {
    ...
    int o = noise3( x_int, y_int, z_int, seed );
    int p = noise3( x_int + 1, y_int, z_int, seed );
    int q = noise3( x_int, y_int + 1, z_int, seed );
    int r = noise3( x_int + 1, y_int + 1, z_int, seed );

    float low1 = smooth_inter( o, p, x_frac );
    float high1 = smooth_inter( q, r, x_frac );
    float down = smooth_inter( low1, high1, y_frac );

    int s = noise3( x_int, y_int, z_int + 1, seed );
    int t = noise3( x_int + 1, y_int, z_int + 1, seed );
    int u = noise3( x_int, y_int + 1, z_int + 1, seed );
    int v = noise3( x_int + 1, y_int + 1, z_int + 1, seed );

    float low2 = smooth_inter( s, t, x_frac );
    float high2 = smooth_inter( u, v, x_frac );
    float up = smooth_inter( low2, high2, y_frac );
    return smooth_inter( down, up, z_frac );
}

float perlin_noise2d( float x, float y, float freq, int depth, int seed ) {
    float xa = x * freq;
    float ya = y * freq;
    float amp = 1.0;
    float fin = 0;
...
float perlin_noise3d( float x, float y, float z, float freq, int depth, int seed ) {
    float xa = x * freq;
    float ya = y * freq;
    float za = z * freq;
    float amp = 1.0;
